...
> **Note:** Pure right-click (without modifier) cannot be captured globally on Wayland without breaking app functionality. `Super+RightClick` is the recommended approach. If you want unmodified right-click, install the optional `hyprbars` plugin and add a button that triggers `hypr-windowmenu`.

### 🎹 Changing the Trigger Key

The trigger key is configured in **Hyprland**, not in this app. You can use **any valid Hyprland modifier**:
...
# Shift + Right Click
bind = SHIFT, mouse:273, exec, hypr-windowmenu
...
Any keybind that works in Hyprland will work here.
